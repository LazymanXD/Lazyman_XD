...
// Two buttons share this role in different layout states (pre-load view and
// in-desktop view). They used to share one duplicate id, which meant
// getElementById only ever found the first one and the second never
// responded to clicks. Now both are controlled together.
const roadmapToggleBtns = document.querySelectorAll(
  "#roadmapToggleBtn, #roadmapToggleBtnDesktop",
);
...
function openUpdatePanel() {
  ...
  roadmapToggleBtns.forEach((btn) => (btn.style.display = "none"));
  closeSettingsDrawer();
...
function closeUpdatePanel() {
  ...
  roadmapToggleBtns.forEach((btn) => (btn.style.display = ""));
  const homeContent = document.getElementById("homeContentOutside");
  const navButtonsContainer = document.getElementById("navButtonsContainer");
...
function showPage(pageKey) {
  ...
  // (mobile and desktop currently use the same rule)
  roadmapToggleBtns.forEach((btn) => {
    btn.style.display = pageKey === "home" ? "" : "none";
  });
...
function closeInfiniteBook() {
  ...
  bookOverlayOpen = false;
  roadmapToggleBtns.forEach((btn) => {
    btn.style.display = lastOpenedPage === "home" ? "" : "none";
  });
...
const mangaCoverImage = "./assets/SHOWERTHOUGHTS.webp";

const mangaInfo = {
  title: "Shower Thoughts",
  cover: "./assets/SHOWERTHOUGHTS.webp",
  synopsis:
...
// NOTE: character1-5 sketch/flat/rendered images were never included in the
// project files, so this gallery currently has nothing to show. Paths are
// fixed to point at /assets (where every other image lives) and now fall
// back to an existing placeholder instead of a broken image icon. Add the
// real files to /assets (e.g. assets/character1_sketch.webp) to populate it.
const qaCharacterImages = {
  1: {
    sketch: "./assets/character1_sketch.webp",
    flat: "./assets/character1_flat.webp",
    rendered: "./assets/character1_rendered.webp",
  },
  2: {
    sketch: "./assets/character2_sketch.webp",
    flat: "./assets/character2_flat.webp",
    rendered: "./assets/character2_rendered.webp",
  },
  3: {
    sketch: "./assets/character3_sketch.webp",
    flat: "./assets/character3_flat.webp",
    rendered: "./assets/character3_rendered.webp",
  },
  4: {
    sketch: "./assets/character4_sketch.webp",
    flat: "./assets/character4_flat.webp",
    rendered: "./assets/character4_rendered.webp",
  },
  5: {
    sketch: "./assets/character5_sketch.webp",
    flat: "./assets/character5_flat.webp",
    rendered: "./assets/character5_rendered.webp",
  },
};
