...
function enableWikiEditing() {
  setTimeout(() => {
    ...
    toolbar.style.cssText = 'position:fixed;top:60px;right:20px;background:rgba(0,0,0,0.9);border:2px solid #ffd700;border-radius:10px;padding:10px;z-index:10000;display:flex;gap:10px;flex-wrap:wrap;max-width:300px';
    toolbar.innerHTML = `
...
function changeFont() {
  ...
  if (choice && choice >= 1 && choice <= fonts.length) document.execCommand('fontName', false, fonts[choice - 1]);
}

function addLink() {
  const url = prompt('Enter URL:', 'https://');
  const text = prompt('Enter link text:', 'Click here');
  if (url && text) document.execCommand('insertHTML', false, `<a href="${url}" target="_blank" style="color:#ffd700;text-decoration:underline;">${text}</a>`);
}
...
function addNewPage() {
  ...
  const newPage = { id: Date.now(), title: pageTitle, content: `<h2>${pageTitle}</h2><p>This is a new wiki page. Start editing!</p>`, created: new Date().toISOString() };
  wikiPages.push(newPage);
  localStorage.setItem('wikiPages', JSON.stringify(wikiPages));
  
  document.execCommand('insertHTML', false, `<p>📄 <a href="#" onclick="openWikiPage(${newPage.id})" style="color:#ffd700;text-decoration:underline;">${pageTitle}</a></p>`);
...
function addImage() {
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(event) {
      document.execCommand('insertHTML', false, `<img src="${event.target.result}" style="max-width:100%;height:auto;margin:10px 0;border-radius:8px;">`);
    };
...
function setupImageDragAndDrop() {
  ...
  wikiMainContent.addEventListener('dragover', e => { e.preventDefault(); wikiMainContent.style.backgroundColor = 'rgba(255, 215, 0, 0.1)'; });
  wikiMainContent.addEventListener('dragleave', e => { e.preventDefault(); wikiMainContent.style.backgroundColor = ''; });
  
  wikiMainContent.addEventListener('drop', e => {
    e.preventDefault();
    wikiMainContent.style.backgroundColor = '';
...
function disableWikiEditing() {
  ...
  if (typeof showPage === 'function') showPage('wiki');
}
...
function updateWikiWithMangaData() {
  ...
  if (wikiContentElement) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => wikiContentElement.classList.add('show'), 250);
    }, 500);
  }
}
