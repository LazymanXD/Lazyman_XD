...
/**
 * Show manga cards in a grid layout
...
/**
 * Hide manga cards with animation
 * @param {HTMLElement} button - Button that triggered the action
 */
...
function openMangaReader(manga) {
  ...
  function renderGrid(imageList, labelPrefix) {
    grid.innerHTML = "";
...
function toggleMangaSection() {
  ...
  if (currentMangaSection === "manga") {
    const illustrationImages = (currentManga.illustrations || [])
      .map((key) => mangaImagePool[key])
      .filter(Boolean);
    ...
    const pages = getMangaPages(currentManga);
    const fragment = document.createDocumentFragment();
